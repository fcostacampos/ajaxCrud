    ...
    function ajaxCRUD($item, $db_table, $db_table_pk, $ajaxcrud_root = "") {
        ...
        foreach ($this->fields as $field){
			if ($_REQUEST[$field] != '' && ($action != 'add' && $action != 'delete' && $action != 'update' && $action != 'upload' && $action != 'delete_file')){
				...
				$filter_value = utf8_encode($_REQUEST[$field]);
            ...
            if ($_REQUEST['table'] == $this->db_table){
                ...
                foreach($this->fields as $field){
                    ...
                    if ($_REQUEST[$field] == ''){
                        ...
                        $submitted_value_cleansed = utf8_encode($_REQUEST[$field]);
                    }
        ...
        if (count($this->ajaxFilter_fields) > 0){
          ...
          if ($this->category_whereclause_array[$found_category_index] != ''){
              ...
              $top_html .= "<option value=\"$dropdown_value\" >$dropdown_text</option>\n";
                	...
                	$top_html .= "<br><input type=\"text\" class=\"$custom_class\" size=\"$textbox_size\" name=\"$filter_field\" value=\"$filter_value\" onKeyUp=\"filterTable(this, '" . $this->db_table . "', '$filter_field', '$extra_query_params');\">";
                }
                $top_html .= "</td>";
            }
            $top_html .= "</tr></table>\n";
            ...
            foreach($this->add_fields as $field){
                if ($field != $this->db_table_pk || $this->on_add_specify_primary_key){
                    ...
                    if (is_array($this->checkbox[$field])){
                        ...
                        $value_on = $values[0];
                        $value_off = $values[1];
                        $add_html .= "<td width='20%'>$display_field</td><td>\n";
